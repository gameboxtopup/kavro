(async function(){
 ...
 document.getElementById("buyButton").onclick=e=>{e.preventDefault();const name=document.getElementById("packageName").textContent;const price=document.getElementById("packagePrice").textContent;if(name==="Select a card"){alert("Please select a Steam Gift Card first.");return;}location.href=`order.html?product=${encodeURIComponent(PRODUCT_NAME)}&package=${encodeURIComponent(name)}&price=${encodeURIComponent(price)}`;};
})();
